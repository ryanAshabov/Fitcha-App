/*
  # Fix Game Requests Migration - Remove Conflicts

  This migration creates the game_requests table and policies while avoiding conflicts
  with existing policies by dropping them first.
...
DO $$
BEGIN
  -- Drop all existing policies on game_requests table if they exist
...
CREATE POLICY "game_requests_select_policy"
  ON game_requests
  FOR SELECT
  TO authenticated
  USING (auth.uid() = sender_id OR auth.uid() = receiver_id);

CREATE POLICY "game_requests_insert_policy"
  ON game_requests
  FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = sender_id);

CREATE POLICY "game_requests_update_receiver_policy"
  ON game_requests
  FOR UPDATE
  TO authenticated
...
CREATE POLICY "game_requests_update_sender_policy"
  ON game_requests
  FOR UPDATE
  TO authenticated
